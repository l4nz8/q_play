...
class GameState:
    def __init__(self, pyboy):
        ...
        mario_x = pyboy.get_memory_value(
            0xC202
        )  # Mario's X position relative to screen
        scx = (
            pyboy.botsupport_manager().screen().tilemap_position_list()[16][0]
        )  # Screen scroll position
        real = (
            (scx - 7) % 16 if (scx - 7) % 16 != 0 else 16
        )  # Adjust for tilemap alignment

        self.real_x_pos = (
            level_block * 16 + real + mario_x
        )  # Compute Mario's absolute X position
        ...
        self._level_progress_max = max(
            game_wrapper._level_progress_max, self.real_x_pos
        )  # Track furthest level progress
...
class MarioAI:
    ...
    def GetReward(self, prevGameState: GameState, pyboy):
        ...
        timeRespawn = pyboy.get_memory_value(
            0xFFA6
        )  # Check if Mario is in respawn state
        if timeRespawn > 0:
        ...
        if max(
            (current_mario.world[0] - prevGameState.world[0]),
            (current_mario.world[1] - prevGameState.world[1]),
        ):
            for _ in range(0, 5):
        ...
        if len(self.realMax) == 0:
            self.realMax.append(
                [
                    current_mario.world[0],
                    current_mario.world[1],
                    current_mario._level_progress_max,
                ]
            )
        else:
            ...
            for elem in self.realMax:  #  Max length
                if (
                    elem[0] == current_mario.world[0]
                    and elem[1] == current_mario.world[1]
                ):
        ...
        levelReward = 15 * max(
            (current_mario.world[0] - prevGameState.world[0]),
            (current_mario.world[1] - prevGameState.world[1]),
        )  # +15 if either new level or new world
    ...
    def GetActions(self):
        ...
        del filteredActions[
            4
        ]  # Remove conflicting action ['PRESS_ARROW_RIGHT', 'PRESS_ARROW_LEFT']
    ...
    def GetLength(self, pyboy):
        ...
        pyboy.game_wrapper()._level_progress_max = (
            0  # Reset max level progress because game hasnt implemented it
        )
        self.realMax = []
        return result
