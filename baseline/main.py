...
parser = argparse.ArgumentParser(
    description="\U0001F916 Train or Play with Mario AI",
...
group.add_argument(
    ...
    type=int,
    default=1,
    help="Choose the game world to play or train in {e.g. 1 for world 1} (default=1)",
)
group.add_argument(
    "--level",
    type=int,
    default=1,
    help="Choose the game level to play or train in {e.g. 1 for Level 1} (default=1)",
)
...
if __name__ == "__main__":
    ...
    )  # Check for CUDA (GPU) availability for PyTorch
    ...
    pyboy = PyBoy(
        gamerom_file="gb_ROM/SuperMarioLand.gb",
    ...
    env = CustomPyBoyGym(
        pyboy, observation_type=observation_types[0], load_initial_state=args.load_state
    )
    ...
    print(
        "Possible actions: ",
        [[WindowEvent(i).__str__() for i in x] for x in filteredActions],
    )
    ...
    env = ResizeObservation(
        env, shape=gameDimentions
    )  # Resize observations to the Box for framestack
    env = FrameStack(
        env, num_stack=frameStack
    )  # Stack frames to create a temporal dimension

    # Load AI agent
    mario = MarioGymAI(
        state_dim=(frameStack,) + gameDimentions,
        action_space_dim=len(filteredActions),
        save_dir=save_dir,
        args=args,
    )
    ...
    if args.mode == "train":
        ...
        for e in range(episodes):
            ...
            while True:
                ...
                mario.cache(
                    state, next_state, actionId, reward, done
            ...
            avg_reward, avg_length, avg_loss, avg_q = mario.update_moving_averages(
                episode_reward, step, avg_loss, avg_q
            )
